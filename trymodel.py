...
def draw_my_plot(xstart, xstop, nbins,  n=70, p=0.5, jpower = 0, out = []):
    if n < 0: 
    ...
    c1 = TCanvas('c1', 'Test', 900, 500)
    c1.Divide(2, 1)

    reader = NBDAnalyticRetreiver('norm1', 'NBD ; counts') 
    h = reader.get_hist(nbins, (xstart, xstop), start=xstart, stop=xstop, n=n, p=p)
    ...
    ROOT.enableJSVis()
    c1.Draw()
    # Dirty hack to avoid root memory management
    out.append(h)
    out.append(g)
    out.append(c1)
...
def draw_all(out = []):
    ...
    f = lambda xstart, xstop, nbins, n, p, jpower: draw_my_plot(xstart, xstop, nbins, n, p, jpower, out)
    w = interactive(f, xstart=xstart_slider, xstop=xstop_slider, nbins=nbins_slider, n=n_slider, p=p_slider, jpower=j_slider)
    return w
